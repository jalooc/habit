...
- **Active hours are wall-clock, not elapsed time.** The day's boundaries are set from `hour`/`minute` components, so 08:00–20:00 means local 08:00–20:00 even on a DST transition day; deriving them by adding minutes to midnight would slide the whole window an hour twice a year. Stepping between days (`.add(_, 'day')`) already preserves wall clock. Sub-minute references matter too — measure against slot boundaries in *fractional* minutes, since a truncated `diff` rounds back onto the occurrence that just fired and sends the search a whole day forward. Known gap: a span crossing the skipped hour (e.g. 22:00–06:00 in spring) is still divided by its nominal length rather than its real one, marked with a failing test in `getOccurrence.test.ts`.
