...
- **The interval is why early ticks count.** Completing anywhere inside a turn serves it, so ticking ahead of the due moment doesn't leave the rotation due again minutes later, and a missed turn stays due until its slot runs out. This is why `isGroupDue` hangs on the most recently due turn's `opensAt` rather than on the occurrence itself: comparing against the occurrence would stop counting a completion made before it. `habits/utils/nextTurnDueAt` answers "when does the next *unserved* turn come due" and is what Home's upcoming rows, the Group screen's next-turn label and `buildNotifications` must use, since calling `getOccurrence.next` directly there would announce a turn that is already done.
